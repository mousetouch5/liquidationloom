            ...
            <main class="flex-1 px-8 py-6 space-y-6 bg-gray-50">
                ...
                <div class="slider-container">
                    <div class="slider" id="slider">
                        <!-- Slider sections will be dynamically generated here -->
                    </div>
                ...
                <script>
                    ...
                    function populateSlider() {
                        $.ajax({
                            url: '/event-data', // Replace with your actual endpoint
                            method: 'GET',
                            dataType: 'json',
                            success: function(data) {
                                ...
                                data.forEach(events => {
                                    const section = $(`
                        <section class="bg-white shadow-lg rounded-lg p-6 flex flex-col" data-aos="fade-up" data-aos-duration="2000">
                            <img src="${events.eventImage}" alt="${events.eventName}" class="rounded-lg w-full mb-4 h-72">
                            <div>
                                <h3 class="text-xl font-semibold">${events.eventName}</h3>
                                <p class="mt-4 text-sm text-gray-500">${events.eventDescription}</p>
                                <p class="mt-2 text-sm text-gray-500"><strong>Date:</strong> ${new Date(events.eventDate).toLocaleDateString()}</p>
                                <p class="mt-2 text-sm text-gray-500"><strong>Time:</strong> ${events.eventTime}</p>
                                <p class="mt-2 text-sm text-gray-500"><strong>Organizer:</strong> ${events.organizer}</p>
                                <p class="mt-2 text-sm text-gray-500"><strong>Location:</strong> ${events.eventLocation}</p>
                            </div>
                        </section>
                    `);
                                ...
                                showSlides(); // Initialize the slider after populating
                            },
                    ...
                    // Populate the slider with event data on page load
